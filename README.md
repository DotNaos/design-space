...
Component folders may choose a Lucide icon with an empty marker file named `.<lucide-kebab-name>.lucide-icon` in that folder—for example, `.git-commit.lucide-icon`. The marker changes only the folder row in the component catalog. It does not create a component or enter the review graph. Missing, unknown, invalid, or multiple markers safely use the normal folder icon.
