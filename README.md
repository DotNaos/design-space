...
Component folders may choose a Lucide icon with an empty marker file named `.<lucide-kebab-name>.lucide-icon` in that folder—for example, `.git-commit.lucide-icon`. Directories containing a `package.json` also show a package marker in the catalog, and remain visible even when they contain only one direct component. These markers change only the folder row in the component catalog. They do not create a component or enter the review graph. Missing, unknown, invalid, or multiple Lucide markers safely use the normal folder icon.
