  ...
  return (
    <Modal.Backdrop isOpen={props.open} onOpenChange={(open) => { if (!open) props.onClose(); }} variant="transparent">
      <Modal.Container className="items-end p-0" placement="bottom" size="lg">
        <Modal.Dialog aria-label={`Add to ${props.slotLabel} slot`} className={`mt-auto w-full overflow-hidden rounded-b-none rounded-t-2xl border border-white/10 bg-[#141518] text-zinc-200 shadow-2xl transition-[height] duration-300 ${expanded ? "h-[82dvh]" : "h-[52dvh]"}`}>
          ...
          <Modal.Body className="min-h-0 p-0">
            <SlotCatalogPanel className="flex h-full" slotLabel={props.slotLabel} entries={props.entries} onClose={props.onClose} onSelect={props.onSelect} />
          </Modal.Body>
        </Modal.Dialog>
      </Modal.Container>
    </Modal.Backdrop>
