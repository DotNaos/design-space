...
it("describes the selected canvas layer from the application root", () => {
  ...
  const shell = { ...occurrence("shell", "WorkspaceShell", "app"), usageSlot: content };
  const toolbar = { ...content, id: "toolbar-slot", label: "toolbar" };
  const graph: SourceFocusGraph = {
    roots: [app.id],
    occurrences: new Map([[app.id, app], [shell.id, shell]]),
  };

  expect(sourceCanvasAncestry(graph, shell.id, toolbar)).toEqual([
    { id: "app", kind: "component", label: "App" },
    { id: "content-slot", kind: "slot", label: "slot:content", scope: "tree" },
    { id: "shell", kind: "component", label: "WorkspaceShell" },
    { id: "toolbar-slot", kind: "slot", label: "slot:toolbar", scope: "tree" },
  ]);
});
