...
export function createDesignIdFactory(
  ...
  return () => {
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const candidate = `draft-${createUuid().toLowerCase()}`;
      if (used.has(candidate)) continue;
      used.add(candidate);
      return candidate;
    }
    throw new Error("A unique local design identity could not be generated.");
  };
}

function createPortableUuid(): string {
  ...
  const visit = (node: DesignComponentNode) => {
    ...
    for (const children of Object.values(node.slots)) {
      for (const child of children) {
        if (child.kind === "component") visit(child.node);
